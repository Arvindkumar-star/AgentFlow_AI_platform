  ...
  return (
    ...
    >
      ...
      <div className="mt-5 space-y-4">
        {displayLogs.map((log) => {
          const req = log.requestedAmount;
          const max = log.maxLimit;
          const isPassing = req <= max;
          // Scale visual width safely (cap scale at 100%)
          const ceilingMax = Math.max(req, max, 10000);
            ...
            >
              <div className="flex items-center justify-between text-xs mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold" style={{ color: 'var(--text-primary)' }}>{log.vendor}</span>
                  <span
              ...
              <div className="flex items-center justify-between text-[10px] font-mono mt-1.5 px-0.5" style={{ color: 'var(--text-muted)' }}>
                <span>Verification: {log.proofType || 'Groth16 / BN128'} ({log.verificationTimeMs} ms)</span>
                <span className={isPassing ? 'text-cyan-600 dark:text-cyan-300 font-semibold' : 'text-rose-500 font-semibold'}>
                  {isPassing
                    ? `✓ Safe margin: ₹${(max - req).toLocaleString()}`
                    : `⚠️ Breach detected: +₹${(req - max).toLocaleString()}`}
                </span>
              </div>
            </div>
          );
